...
#[test]
fn encode__cmn_register_t1__register_to_register() {
    // CMN r0, r1
    let instruction = ArmT32Instruction::Cmn_Register_T1(
        Arm32LowGeneralPurposeRegister::R0,
        Arm32LowGeneralPurposeRegister::R1,
    );
    //
    let encoded_instruction = instruction.encode().ok().unwrap();
    assert_eq!(encoded_instruction, vec![0xC8, 0x42]);
}

#[test]
fn decode__cmn_register_t1__register_to_register() {
    // CMN r0, r1
    let encoded_instruction = [0xC8, 0x42];
    let verify_instruction = ArmT32Instruction::Cmn_Register_T1(
        Arm32LowGeneralPurposeRegister::R0,
        Arm32LowGeneralPurposeRegister::R1,
    );
    //
