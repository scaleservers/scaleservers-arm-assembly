...
#[test]
fn encode__uxth_t1__register_to_register() {
    // UXTH r0, r1
    let instruction = ArmT32Instruction::Uxth_T1(
        Arm32LowGeneralPurposeRegister::R0,
        Arm32LowGeneralPurposeRegister::R1,
    );
    //
    let encoded_instruction = instruction.encode().ok().unwrap();
    assert_eq!(encoded_instruction, vec![0x88, 0xB2]);
}

#[test]
fn decode__uxth_t1__register_to_register() {
    // UXTH r0, r1
    let encoded_instruction = [0x88, 0xB2];
    let verify_instruction = ArmT32Instruction::Uxth_T1(
        Arm32LowGeneralPurposeRegister::R0,
        Arm32LowGeneralPurposeRegister::R1,
    );
    //
