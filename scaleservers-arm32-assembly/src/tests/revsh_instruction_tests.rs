...
#[test]
fn encode__revsh_t1__register_to_register() {
    // REVSH r0, r1
    let instruction = ArmT32Instruction::Revsh_T1(
        Arm32LowGeneralPurposeRegister::R0,
        Arm32LowGeneralPurposeRegister::R1,
    );
    //
    let encoded_instruction = instruction.encode().ok().unwrap();
    assert_eq!(encoded_instruction, vec![0xC8, 0xBA]);
}

#[test]
fn decode__revsh_t1__register_to_register() {
    // REVSH r0, r1
    let encoded_instruction = [0xC8, 0xBA];
    let verify_instruction = ArmT32Instruction::Revsh_T1(
        Arm32LowGeneralPurposeRegister::R0,
        Arm32LowGeneralPurposeRegister::R1,
    );
    //
