...
#[test]
fn encode__mul_t1__register_to_register() {
    // MULS r0, r1
    let instruction = ArmT32Instruction::Mul_T1(
        Arm32LowGeneralPurposeRegister::R0,
        Arm32LowGeneralPurposeRegister::R1,
    );
    //
    let encoded_instruction = instruction.encode().ok().unwrap();
    assert_eq!(encoded_instruction, vec![0x48, 0x43]);
}

#[test]
fn decode__mul_t1__register_to_register() {
    // MULS r0, r1
    let encoded_instruction = [0x48, 0x43];
    let verify_instruction = ArmT32Instruction::Mul_T1(
        Arm32LowGeneralPurposeRegister::R0,
        Arm32LowGeneralPurposeRegister::R1,
    );
    //
