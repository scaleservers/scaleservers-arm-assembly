...
#[test]
fn encode__sbc_register_t1__register_to_register() {
    // SBCS r0, r1
    let instruction = ArmT32Instruction::Sbc_Register_T1(
        Arm32LowGeneralPurposeRegister::R0,
        Arm32LowGeneralPurposeRegister::R1,
    );
    //
    let encoded_instruction = instruction.encode().ok().unwrap();
    assert_eq!(encoded_instruction, vec![0x88, 0x41]);
}

#[test]
fn decode__sbc_register_t1__register_to_register() {
    // SBCS r0, r1
    let encoded_instruction = [0x88, 0x41];
    let verify_instruction = ArmT32Instruction::Sbc_Register_T1(
        Arm32LowGeneralPurposeRegister::R0,
        Arm32LowGeneralPurposeRegister::R1,
    );
    //
