...
#[test]
fn encode__rev16_t1__register_to_register() {
    // REV16 r0, r1
    let instruction = ArmT32Instruction::Rev16_T1(
        Arm32LowGeneralPurposeRegister::R0,
        Arm32LowGeneralPurposeRegister::R1,
    );
    //
    let encoded_instruction = instruction.encode().ok().unwrap();
    assert_eq!(encoded_instruction, vec![0x48, 0xBA]);
}

#[test]
fn decode__rev16_t1__register_to_register() {
    // REV16 r0, r1
    let encoded_instruction = [0x48, 0xBA];
    let verify_instruction = ArmT32Instruction::Rev16_T1(
        Arm32LowGeneralPurposeRegister::R0,
        Arm32LowGeneralPurposeRegister::R1,
    );
    //
