...
#[test]
fn encode__sxtb_t1__register_to_register() {
    // SXTB r0, r1
    let instruction = ArmT32Instruction::Sxtb_T1(
        Arm32LowGeneralPurposeRegister::R0,
        Arm32LowGeneralPurposeRegister::R1,
    );
    //
    let encoded_instruction = instruction.encode().ok().unwrap();
    assert_eq!(encoded_instruction, vec![0x48, 0xB2]);
}

#[test]
fn decode__sxtb_t1__register_to_register() {
    // SXTB r0, r1
    let encoded_instruction = [0x48, 0xB2];
    let verify_instruction = ArmT32Instruction::Sxtb_T1(
        Arm32LowGeneralPurposeRegister::R0,
        Arm32LowGeneralPurposeRegister::R1,
    );
    //
