...
#[test]
fn encode__ldrsb_register_t1__register_and_register_to_register() {
    // LDRSB r0, [r1, r2]
    let instruction = ArmT32Instruction::Ldrsb_Register_T1(
    ...
    assert_eq!(encoded_instruction, vec![0x88, 0x56]);
}

#[test]
fn decode__ldrsb_register_t1__register_and_register_to_register() {
    // LDRSB r0, [r1, r2]
    let encoded_instruction = [0x88, 0x56];
    let verify_instruction = ArmT32Instruction::Ldrsb_Register_T1(
        Arm32LowGeneralPurposeRegister::R0,
        Arm32LowGeneralPurposeRegister::R1,
        Arm32LowGeneralPurposeRegister::R2,
    );
