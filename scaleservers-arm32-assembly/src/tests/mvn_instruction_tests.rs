...
#[test]
fn encode__mvn_register_t1__register_to_register() {
    // MVNS r0, r1
    let instruction = ArmT32Instruction::Mvn_Register_T1(
        Arm32LowGeneralPurposeRegister::R0,
        Arm32LowGeneralPurposeRegister::R1,
    );
    //
    let encoded_instruction = instruction.encode().ok().unwrap();
    assert_eq!(encoded_instruction, vec![0xC8, 0x43]);
}

#[test]
fn decode__mvn_register_t1__register_to_register() {
    // MVNS r0, r1
    let encoded_instruction = [0xC8, 0x43];
    let verify_instruction = ArmT32Instruction::Mvn_Register_T1(
        Arm32LowGeneralPurposeRegister::R0,
        Arm32LowGeneralPurposeRegister::R1,
    );
    //
