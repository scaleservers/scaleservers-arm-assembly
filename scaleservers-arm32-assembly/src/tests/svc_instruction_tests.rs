...
#[test]
fn encode__svc_t1__immediate() {
    // SVC #170
    let instruction = ArmT32Instruction::Svc_T1(170);
    //
    let encoded_instruction = instruction.encode().ok().unwrap();
    assert_eq!(encoded_instruction, vec![0xAA, 0xDF]);
}

#[test]
fn decode__svc_t1__immediate() {
    // SVC #170
    let encoded_instruction = [0xAA, 0xDF];
    let verify_instruction = ArmT32Instruction::Svc_T1(170);
    //
