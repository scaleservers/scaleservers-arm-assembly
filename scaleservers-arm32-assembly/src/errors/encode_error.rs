...
pub enum EncodeError {
    ...
    UnsupportedInstructionForTarget {
        required: ArmInstructionRequirement,
        target_isa_version: ArmIsaVersion,
    },
}
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::ConditionNotEncodable { field, detail } => {
                write!(
                    formatter,
                    "condition in operand `{field}` is not encodable here: {detail}"
                )
            }
            Self::ShiftNotEncodable { field, detail } => {
                write!(
                    formatter,
                    "shift in operand `{field}` is not encodable here: {detail}"
                )
            }
            Self::ModifiedImmediateNotEncodable { field, value } => {
                write!(
                    formatter,
                    "operand `{field}` constant 0x{value:08X} cannot be expressed as a Thumb modified immediate"
                )
            }
