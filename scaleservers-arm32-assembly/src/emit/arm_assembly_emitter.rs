...
impl ArmT32Instruction {
    ...
    fn render(&self, instruction_address: Option<u32>, syntax: ArmAssemblySyntax) -> String {
        match self {
            ...
            Self::Add_Immediate_T1(rd, rn, imm3) => format!("adds {}, {}, {}", low(rd), low(rn), imm(syntax, *imm3 as i64)),
            Self::Add_Immediate_T2(rdn, imm8) => format!("adds {}, {}", low(rdn), imm(syntax, *imm8 as i64)),
            Self::Add_Register_T1(rd, rn, rm) => format!("adds {}, {}, {}", low(rd), low(rn), low(rm)),
            Self::Add_Register_T2(rdn, rm) => format!("add {}, {}", gpr(rdn), gpr(rm)),
            Self::And_Register_T1(rdn, rm) => format!("ands {}, {}", low(rdn), low(rm)),
            Self::Asr_Immediate_T1(rd, rm, decoded_imm5) => format!("asrs {}, {}, {}", low(rd), low(rm), imm(syntax, *decoded_imm5 as i64)),
            ...
            Self::Lsl_Immediate_T1(rd, rm, imm5) => format!("lsls {}, {}, {}", low(rd), low(rm), imm(syntax, *imm5 as i64)),
            Self::Lsl_Register_T1(rdn, rm) => format!("lsls {}, {}", low(rdn), low(rm)),
            Self::Lsr_Immediate_T1(rd, rm, decoded_imm5) => format!("lsrs {}, {}, {}", low(rd), low(rm), imm(syntax, *decoded_imm5 as i64)),
            Self::Lsr_Register_T1(rdn, rm) => format!("lsrs {}, {}", low(rdn), low(rm)),
            Self::Mov_Immediate_T1(rd, imm8) => format!("movs {}, {}", low(rd), imm(syntax, *imm8 as i64)),
            ...
            Self::Sub_Immediate_T1(rd, rn, imm3) => format!("subs {}, {}, {}", low(rd), low(rn), imm(syntax, *imm3 as i64)),
            Self::Sub_Immediate_T2(rdn, imm8) => format!("subs {}, {}", low(rdn), imm(syntax, *imm8 as i64)),
            Self::Sub_Register_T1(rd, rn, rm) => format!("subs {}, {}, {}", low(rd), low(rn), low(rm)),
            Self::Cmn_Register_T1(rn, rm) => format!("cmn {}, {}", low(rn), low(rm)),
            Self::Cmp_Immediate_T1(rn, imm8) => format!("cmp {}, {}", low(rn), imm(syntax, *imm8 as i64)),
            ...
            Self::Add_SpPlusImmediate_T1(rd, const10) => format!("add {}, sp, {}", low(rd), imm(syntax, *const10 as i64)),
            Self::Add_SpPlusImmediate_T2(const9) => format!("add sp, sp, {}", imm(syntax, *const9 as i64)),
            Self::Add_SpPlusRegister_T1(m) => format!("add {}, sp, {}", gpr(m), gpr(m)),
            Self::Add_SpPlusRegister_T2(rm) => format!("add sp, {}", gpr(rm)),
            Self::Sub_SpMinusImmediate_T1(const9) => format!("sub sp, sp, {}", imm(syntax, *const9 as i64)),
            Self::Adr_T1(rd, const10) => {
                match instruction_address {
                    Some(address) => format!("adr {}, 0x{:08x}", low(rd), pc_relative_target(address, *const10 as i64, true)),
                    None => format!("adr {}, {}", low(rd), imm(syntax, *const10 as i64)),
                }
            },

            // ---- branches ----
            Self::B_T1(cond, decoded_signed_imm9) => render_branch(&format!("b{}", cond_suffix(cond)), instruction_address, *decoded_signed_imm9 as i64, syntax),
            Self::B_T2(decoded_signed_imm12) => render_branch("b", instruction_address, *decoded_signed_imm12 as i64, syntax),
            Self::Bl_T1(decoded_signed_imm25) => render_branch("bl", instruction_address, *decoded_signed_imm25 as i64, syntax),
            // M7m wide branches and compare-and-branch
            Self::B_T4(offset) => render_branch("b.w", instruction_address, *offset as i64, syntax),
            Self::B_T3(cond, offset) => render_branch(&format!("b{}.w", cond_suffix(cond)), instruction_address, *offset as i64, syntax),
            Self::Cbz_T1(rn, offset) => render_compare_branch("cbz", low(rn), instruction_address, *offset as i64, syntax),
            Self::Cbnz_T1(rn, offset) => render_compare_branch("cbnz", low(rn), instruction_address, *offset as i64, syntax),
            ...
            Self::Sxtab_T1(rd, rn, rm, rot) => format!("sxtab {}, {}, {}{}", gpr(rd), gpr(rn), gpr(rm), render_rotation(*rot)),
            Self::Uxtab_T1(rd, rn, rm, rot) => format!("uxtab {}, {}, {}{}", gpr(rd), gpr(rn), gpr(rm), render_rotation(*rot)),
            Self::Sxtah_T1(rd, rn, rm, rot) => format!("sxtah {}, {}, {}{}", gpr(rd), gpr(rn), gpr(rm), render_rotation(*rot)),
            Self::Uxtah_T1(rd, rn, rm, rot) => format!("uxtah {}, {}, {}{}", gpr(rd), gpr(rn), gpr(rm), render_rotation(*rot)),
            Self::Sxtab16_T1(rd, rn, rm, rot) => format!("sxtab16 {}, {}, {}{}", gpr(rd), gpr(rn), gpr(rm), render_rotation(*rot)),
            Self::Uxtab16_T1(rd, rn, rm, rot) => format!("uxtab16 {}, {}, {}{}", gpr(rd), gpr(rn), gpr(rm), render_rotation(*rot)),
            Self::Sxtb16_T1(rd, rm, rot) => format!("sxtb16 {}, {}{}", gpr(rd), gpr(rm), render_rotation(*rot)),
            Self::Uxtb16_T1(rd, rm, rot) => format!("uxtb16 {}, {}{}", gpr(rd), gpr(rm), render_rotation(*rot)),

            // ---- ARMv7E-M DSP M8c: pack / saturate16 / select / SAD ----
            Self::Pkhbt_T1(rd, rn, rm, lsl) => if *lsl == 0 {
                format!("pkhbt {}, {}, {}", gpr(rd), gpr(rn), gpr(rm))
            } else {
                format!("pkhbt {}, {}, {}, lsl #{}", gpr(rd), gpr(rn), gpr(rm), lsl)
            },
            Self::Pkhtb_T1(rd, rn, rm, asr) => format!("pkhtb {}, {}, {}, asr #{}", gpr(rd), gpr(rn), gpr(rm), asr),
            Self::Ssat16_T1(rd, sat_imm, rn) => format!("ssat16 {}, {}, {}", gpr(rd), imm(syntax, *sat_imm as i64), gpr(rn)),
            Self::Usat16_T1(rd, sat_imm, rn) => format!("usat16 {}, {}, {}", gpr(rd), imm(syntax, *sat_imm as i64), gpr(rn)),
            Self::Sel_T1(rd, rn, rm) => format!("sel {}, {}, {}", gpr(rd), gpr(rn), gpr(rm)),
            Self::Usad8_T1(rd, rn, rm) => format!("usad8 {}, {}, {}", gpr(rd), gpr(rn), gpr(rm)),
            Self::Usada8_T1(rd, rn, rm, ra) => format!("usada8 {}, {}, {}, {}", gpr(rd), gpr(rn), gpr(rm), gpr(ra)),

            // ---- ARMv7E-M DSP M8d: parallel add/subtract (mnemonic = prefix + operation) ----
            Self::ParallelAddSub_T1(operation, prefix, rd, rn, rm) => format!("{}{} {}, {}, {}", prefix.mnemonic(), operation.mnemonic(), gpr(rd), gpr(rn), gpr(rm)),

            // ---- ARMv7E-M DSP M8e: signed multiplies (the b/t halves and x/r suffixes are in the mnemonic) ----
            Self::Smul_T1(rd, rn, rm, n, m) => format!("smul{}{} {}, {}, {}", top_or_bottom(*n), top_or_bottom(*m), gpr(rd), gpr(rn), gpr(rm)),
            Self::Smulw_T1(rd, rn, rm, m) => format!("smulw{} {}, {}, {}", top_or_bottom(*m), gpr(rd), gpr(rn), gpr(rm)),
            Self::Smla_T1(rd, rn, rm, ra, n, m) => format!("smla{}{} {}, {}, {}, {}", top_or_bottom(*n), top_or_bottom(*m), gpr(rd), gpr(rn), gpr(rm), gpr(ra)),
            Self::Smlaw_T1(rd, rn, rm, ra, m) => format!("smlaw{} {}, {}, {}, {}", top_or_bottom(*m), gpr(rd), gpr(rn), gpr(rm), gpr(ra)),
            Self::Smlal_Halfword_T1(rdlo, rdhi, rn, rm, n, m) => format!("smlal{}{} {}, {}, {}, {}", top_or_bottom(*n), top_or_bottom(*m), gpr(rdlo), gpr(rdhi), gpr(rn), gpr(rm)),
            Self::Smuad_T1(rd, rn, rm, x) => format!("smuad{} {}, {}, {}", exchange(*x), gpr(rd), gpr(rn), gpr(rm)),
            Self::Smusd_T1(rd, rn, rm, x) => format!("smusd{} {}, {}, {}", exchange(*x), gpr(rd), gpr(rn), gpr(rm)),
            Self::Smlad_T1(rd, rn, rm, ra, x) => format!("smlad{} {}, {}, {}, {}", exchange(*x), gpr(rd), gpr(rn), gpr(rm), gpr(ra)),
            Self::Smlsd_T1(rd, rn, rm, ra, x) => format!("smlsd{} {}, {}, {}, {}", exchange(*x), gpr(rd), gpr(rn), gpr(rm), gpr(ra)),
            Self::Smlald_T1(rdlo, rdhi, rn, rm, x) => format!("smlald{} {}, {}, {}, {}", exchange(*x), gpr(rdlo), gpr(rdhi), gpr(rn), gpr(rm)),
            Self::Smlsld_T1(rdlo, rdhi, rn, rm, x) => format!("smlsld{} {}, {}, {}, {}", exchange(*x), gpr(rdlo), gpr(rdhi), gpr(rn), gpr(rm)),
            Self::Smmul_T1(rd, rn, rm, round) => format!("smmul{} {}, {}, {}", rounded(*round), gpr(rd), gpr(rn), gpr(rm)),
            Self::Smmla_T1(rd, rn, rm, ra, round) => format!("smmla{} {}, {}, {}, {}", rounded(*round), gpr(rd), gpr(rn), gpr(rm), gpr(ra)),
            Self::Smmls_T1(rd, rn, rm, ra, round) => format!("smmls{} {}, {}, {}, {}", rounded(*round), gpr(rd), gpr(rn), gpr(rm), gpr(ra)),

            // ---- ARMv7E-M FP M8f: load/store ----
            Self::Vldr_Single_T2(sd, rn, off) => format!("vldr {}, {}", single(sd), render_fp_mem(rn, *off as i64, syntax)),
            Self::Vstr_Single_T2(sd, rn, off) => format!("vstr {}, {}", single(sd), render_fp_mem(rn, *off as i64, syntax)),
            Self::Vldr_Double_T1(dd, rn, off) => format!("vldr {}, {}", double(dd), render_fp_mem(rn, *off as i64, syntax)),
            Self::Vstr_Double_T1(dd, rn, off) => format!("vstr {}, {}", double(dd), render_fp_mem(rn, *off as i64, syntax)),
            ...
            Self::Vldm_Single_T2(rn, wb, db, first, count) => {
                ...
                if is_sp_register(rn) && *wb && !*db { format!("vpop {}", list) }
                else { format!("{} {}{}, {}", if *db { "vldmdb" } else { "vldmia" }, gpr(rn), writeback_suffix(*wb), list) }
            },
            Self::Vstm_Single_T2(rn, wb, db, first, count) => {
                let list = fp_single_range(first, *count);
                if is_sp_register(rn) && *wb && *db { format!("vpush {}", list) }
                else { format!("{} {}{}, {}", if *db { "vstmdb" } else { "vstmia" }, gpr(rn), writeback_suffix(*wb), list) }
            },
            ...
            Self::FldmdbxFstmdbx_T1(load, rn, first, count) =>
                format!("{} {}!, {}", if *load { "fldmdbx" } else { "fstmdbx" }, gpr(rn), fp_double_range(first, *count)),
            // generic coprocessor instructions
            Self::Coproc_Mcr_T1(two, load, cp, opc1, rt, crn, crm, opc2) =>
                format!("{}{} p{}, {}, {}, c{}, c{}, {}", if *load { "mrc" } else { "mcr" }, if *two { "2" } else { "" }, cp, opc1, gpr(rt), crn, crm, opc2),
            Self::Coproc_Cdp_T1(two, cp, opc1, crd, crn, crm, opc2) =>
                format!("cdp{} p{}, {}, c{}, c{}, c{}, {}", if *two { "2" } else { "" }, cp, opc1, crd, crn, crm, opc2),
            Self::Coproc_Mcrr_T1(two, load, cp, opc1, rt, rt2, crm) =>
                format!("{}{} p{}, {}, {}, {}, c{}", if *load { "mrrc" } else { "mcrr" }, if *two { "2" } else { "" }, cp, opc1, gpr(rt), gpr(rt2), crm),
            Self::Coproc_Ldc_T1(two, long, load, cp, crd, rn, offset) => {
                let mem = if *offset == 0 { format!("[{}]", gpr(rn)) } else { format!("[{}, #{}]", gpr(rn), offset) };
                format!("{}{}{} p{}, c{}, {}", if *load { "ldc" } else { "stc" }, if *two { "2" } else { "" }, if *long { "l" } else { "" }, cp, crd, mem)
            },
            // PACBTI: BTI has no operands; PAC/AUT/PACBTI use the architectural R12, LR, SP.
            Self::PacbtiHint_T1(kind) => match kind { 0 => "bti".to_string(), 1 => "pac r12, lr, sp".to_string(), 2 => "aut r12, lr, sp".to_string(), _ => "pacbti r12, lr, sp".to_string() },
            Self::PacbtiData_T1(op, rd, rn, rm) => format!("{} {}, {}, {}", ["pacg", "autg", "bxaut"][*op as usize], gpr(rd), gpr(rn), gpr(rm)),
            Self::Vscclrm_T1(double, first, count) => {
                ...
                if *count == 0 {
                    ...
                    format!("vscclrm {{s{}-s{}, vpr}}", first, *first as u16 + *count as u16 - 1)
                }
            },
            // CDE: mnemonic = cx{1,2,3} + (d if dual) + (a if accumulate); the dual form names the Rd:Rd+1 pair.
            Self::Cde_Cx1_T1(acc, dual, cp, rd, imm) => {
                let m = format!("cx1{}{}", if *dual { "d" } else { "" }, if *acc { "a" } else { "" });
                let next = crate::enums::Arm32GeneralPurposeRegister::from_operand_bits(rd.as_operand_bits() + 1);
                if *dual { format!("{} p{}, {}, {}, #{}", m, cp, gpr(rd), gpr(&next), imm) }
                else { format!("{} p{}, {}, #{}", m, cp, gpr(rd), imm) }
            },
            Self::Cde_Cx2_T1(acc, dual, cp, rd, rn, imm) => {
                let m = format!("cx2{}{}", if *dual { "d" } else { "" }, if *acc { "a" } else { "" });
                let next = crate::enums::Arm32GeneralPurposeRegister::from_operand_bits(rd.as_operand_bits() + 1);
                if *dual { format!("{} p{}, {}, {}, {}, #{}", m, cp, gpr(rd), gpr(&next), gpr(rn), imm) }
                else { format!("{} p{}, {}, {}, #{}", m, cp, gpr(rd), gpr(rn), imm) }
            },
            Self::Cde_Cx3_T1(acc, dual, cp, rd, rn, rm, imm) => {
                let m = format!("cx3{}{}", if *dual { "d" } else { "" }, if *acc { "a" } else { "" });
                let next = crate::enums::Arm32GeneralPurposeRegister::from_operand_bits(rd.as_operand_bits() + 1);
                if *dual { format!("{} p{}, {}, {}, {}, {}, #{}", m, cp, gpr(rd), gpr(&next), gpr(rn), gpr(rm), imm) }
                else { format!("{} p{}, {}, {}, {}, #{}", m, cp, gpr(rd), gpr(rn), gpr(rm), imm) }
            },
            ...
            Self::Bf_T1(boff, offset) => format!("bf {}, {}", bf_blabel(instruction_address, *boff, syntax), bf_target(instruction_address, *offset as i64, syntax)),
            Self::Bfl_T4(boff, offset) => format!("bfl {}, {}", bf_blabel(instruction_address, *boff, syntax), bf_target(instruction_address, *offset as i64, syntax)),
            Self::Bfx_T3(boff, rn) => format!("bfx {}, {}", bf_blabel(instruction_address, *boff, syntax), gpr(rn)),
            Self::Bflx_T5(boff, rn) => format!("bflx {}, {}", bf_blabel(instruction_address, *boff, syntax), gpr(rn)),
            Self::Bfcsel_T2(boff, offset, cond, t) => {
                ...
                format!("bfcsel {}, {}, {}, {}", bf_blabel(instruction_address, *boff, syntax), bf_target(instruction_address, *offset as i64, syntax), ba, c)
            },

            // ---- CDE VCX1/VCX2/VCX3 (FP/vector custom datapath) ----
            Self::Vcx1_T1(acc, kind, cp, rd, imm) => format!("vcx1{} p{}, {}, #{}", if *acc { "a" } else { "" }, cp, vcx_reg(*kind, *rd), imm),
            Self::Vcx2_T1(acc, kind, cp, rd, rn, imm) => format!("vcx2{} p{}, {}, {}, #{}", if *acc { "a" } else { "" }, cp, vcx_reg(*kind, *rd), vcx_reg(*kind, *rn), imm),
            Self::Vcx3_T1(acc, kind, cp, rd, rn, rm, imm) => format!("vcx3{} p{}, {}, {}, {}, #{}", if *acc { "a" } else { "" }, cp, vcx_reg(*kind, *rd), vcx_reg(*kind, *rn), vcx_reg(*kind, *rm), imm),

            // ---- ARMv7E-M FP M8h: data-processing ----
            Self::FpDataProcess3_Single(op, sd, sn, sm) => format!("{}.f32 {}, {}, {}", op.mnemonic(), single(sd), single(sn), single(sm)),
            Self::FpDataProcess3_Double(op, dd, dn, dm) => format!("{}.f64 {}, {}, {}", op.mnemonic(), double(dd), double(dn), double(dm)),
            Self::FpDataProcess2_Single(op, sd, sm) => format!("{}.f32 {}, {}", op.mnemonic(), single(sd), single(sm)),
            Self::Vmovx_T1(insert, sd, sm) => format!("{}.f16 {}, {}", if *insert { "vins" } else { "vmovx" }, single(sd), single(sm)),
            ...
            Self::Clrm_T1(list) => {
                ...
                for i in 0..=12u8 { if list & (1 << i) != 0 { parts.push(format!("r{}", i)); } }
                if list & (1 << 14) != 0 { parts.push("lr".to_string()); }
                if list & (1 << 15) != 0 { parts.push("apsr".to_string()); }
                format!("clrm {{{}}}", parts.join(", "))
            },
            Self::Vsel_Single_T1(cond, sd, sn, sm) => format!("vsel{}.f32 {}, {}, {}", ["eq", "vs", "ge", "gt"][*cond as usize], single(sd), single(sn), single(sm)),
            Self::Vsel_Double_T1(cond, dd, dn, dm) => format!("vsel{}.f64 {}, {}, {}", ["eq", "vs", "ge", "gt"][*cond as usize], double(dd), double(dn), double(dm)),
            Self::Csel_T1(op, rd, rn, rm, cond) => {
                ...
                let inv = crate::enums::ArmT32InstructionCondition::from_operand_bits(cond.as_operand_bits() ^ 1);
                match (*op, same, is_pc) {
                    (1, true, true)  => format!("cset {}, {}", gpr(rd), inv.ual_suffix()),
                    (2, true, true)  => format!("csetm {}, {}", gpr(rd), inv.ual_suffix()),
                    (1, true, false) => format!("cinc {}, {}, {}", gpr(rd), gpr(rn), inv.ual_suffix()),
                    (2, true, false) => format!("cinv {}, {}, {}", gpr(rd), gpr(rn), inv.ual_suffix()),
                    (3, true, _)     => format!("cneg {}, {}, {}", gpr(rd), gpr(rn), inv.ual_suffix()),
                    _ => format!("{} {}, {}, {}, {}", ["csel", "csinc", "csinv", "csneg"][*op as usize], gpr(rd), gpr(rn), gpr(rm), cond.ual_suffix()),
                }
            },
            Self::LongShiftImm_T1(op, rdalo, rdahi, imm) =>
                format!("{} {}, {}, #{}", ["lsll", "lsrl", "asrl"][*op as usize], gpr(rdalo), gpr(rdahi), imm),
            Self::LongShiftReg_T1(op, rdalo, rdahi, rm) =>
                format!("{} {}, {}, {}", ["lsll", "lsrl", "asrl"][*op as usize], gpr(rdalo), gpr(rdahi), gpr(rm)),
            Self::SatShiftImm_T1(op, rda, imm) =>
                format!("{} {}, #{}", ["uqshl", "urshr", "srshr", "sqshl"][*op as usize], gpr(rda), imm),
            Self::SatShiftLongImm_T1(op, rdalo, rdahi, imm) =>
                format!("{} {}, {}, #{}", ["uqshll", "urshrl", "srshrl", "sqshll"][*op as usize], gpr(rdalo), gpr(rdahi), imm),
            Self::SatShiftReg_T1(signed, rda, rm) =>
                format!("{} {}, {}", if *signed { "sqrshr" } else { "uqrshl" }, gpr(rda), gpr(rm)),
            Self::SatShiftLongReg_T1(signed, rdalo, rdahi, rm, sat48) =>
                format!("{} {}, {}, #{}, {}", if *signed { "sqrshrl" } else { "uqrshll" }, gpr(rdalo), gpr(rdahi), if *sat48 { 48 } else { 64 }, gpr(rm)),
            ...
            Self::Vmaxnm_Single_T1(sd, sn, sm) => format!("vmaxnm.f32 {}, {}, {}", single(sd), single(sn), single(sm)),
            Self::Vmaxnm_Double_T1(dd, dn, dm) => format!("vmaxnm.f64 {}, {}, {}", double(dd), double(dn), double(dm)),
            Self::Vminnm_Single_T1(sd, sn, sm) => format!("vminnm.f32 {}, {}, {}", single(sd), single(sn), single(sm)),
            Self::Vminnm_Double_T1(dd, dn, dm) => format!("vminnm.f64 {}, {}, {}", double(dd), double(dn), double(dm)),
            Self::Vrint_Directed_Single_T1(mode, sd, sm) => format!("vrint{}.f32 {}, {}", t32_directed_round(*mode), single(sd), single(sm)),
            Self::Vrint_Directed_Double_T1(mode, dd, dm) => format!("vrint{}.f64 {}, {}", t32_directed_round(*mode), double(dd), double(dm)),
            ...
            Self::Vcvt_Directed_FromSingle_T1(mode, sd, sm, signed) => format!("vcvt{}.{}.f32 {}, {}", t32_directed_round(*mode), if *signed { "s32" } else { "u32" }, single(sd), single(sm)),
            Self::Vcvt_Directed_FromDouble_T1(mode, sd, dm, signed) => format!("vcvt{}.{}.f64 {}, {}", t32_directed_round(*mode), if *signed { "s32" } else { "u32" }, single(sd), double(dm)),
            Self::Vjcvt_T1(sd, dm) => format!("vjcvt.s32.f64 {}, {}", single(sd), double(dm)),
            Self::FpDataProcess2_Double(op, dd, dm) => format!("{}.f64 {}, {}", op.mnemonic(), double(dd), double(dm)),

            // ---- ARMv7E-M FP M8i: compare / FPSCR transfer / core<->FP move ----
            Self::Vcmp_Single_T1(sd, sm, e) => format!("vcmp{}.f32 {}, {}", if *e { "e" } else { "" }, single(sd), single(sm)),
            Self::Vcmp_Double_T1(dd, dm, e) => format!("vcmp{}.f64 {}, {}", if *e { "e" } else { "" }, double(dd), double(dm)),
            Self::Vcmp_Zero_Single_T2(sd, e) => format!("vcmp{}.f32 {}, #0.0", if *e { "e" } else { "" }, single(sd)),
            Self::Vcmp_Zero_Double_T2(dd, e) => format!("vcmp{}.f64 {}, #0.0", if *e { "e" } else { "" }, double(dd)),
            ...
            Self::Vmov_Core_To_Scalar_T1(size, index, dd, rt) => format!("vmov.{} {}[{}], {}", size.suffix(), double(dd), index, gpr(rt)),
            ...
            Self::Vcvt_FloatToInt_FromSingle_T1(sd, sm, signed, round) => format!("vcvt{}.{}.f32 {}, {}", if *round { "" } else { "r" }, int_type(*signed), single(sd), single(sm)),
            Self::Vcvt_FloatToInt_FromDouble_T1(sd, dm, signed, round) => format!("vcvt{}.{}.f64 {}, {}", if *round { "" } else { "r" }, int_type(*signed), single(sd), double(dm)),
            Self::Vcvt_IntToFloat_ToSingle_T1(sd, sm, signed) => format!("vcvt.f32.{} {}, {}", int_type(*signed), single(sd), single(sm)),
            Self::Vcvt_IntToFloat_ToDouble_T1(dd, sm, signed) => format!("vcvt.f64.{} {}, {}", int_type(*signed), double(dd), single(sm)),
            Self::Vcvt_Single_To_Double_T1(dd, sm) => format!("vcvt.f64.f32 {}, {}", double(dd), single(sm)),
            Self::Vcvt_Double_To_Single_T1(sd, dm) => format!("vcvt.f32.f64 {}, {}", single(sd), double(dm)),

            // ---- ARMv7E-M FP M8i (final corners) ----
            Self::Vmov_Immediate_Single_T1(sd, imm8) => format!("vmov.f32 {}, #{:?}", single(sd), crate::vfp_expand_imm8_to_f32(*imm8) as f64),
            Self::Vmov_Immediate_Double_T1(dd, imm8) => format!("vmov.f64 {}, #{:?}", double(dd), crate::vfp_expand_imm8_to_f64(*imm8)),
            Self::Vmov_CorePair_To_Double_T1(dm, rt, rt2) => format!("vmov {}, {}, {}", double(dm), gpr(rt), gpr(rt2)),
            Self::Vmov_Double_To_CorePair_T1(rt, rt2, dm) => format!("vmov {}, {}, {}", gpr(rt), gpr(rt2), double(dm)),
            Self::Vmov_CorePair_To_Singles_T1(sm, rt, rt2) => format!("vmov s{}, s{}, {}, {}", sm.number(), sm.number() + 1, gpr(rt), gpr(rt2)),
            Self::Vmov_Singles_To_CorePair_T1(rt, rt2, sm) => format!("vmov {}, {}, s{}, s{}", gpr(rt), gpr(rt2), sm.number(), sm.number() + 1),
            Self::Vcvt_HalfToSingle_T1(sd, sm, top) => format!("vcvt{}.f32.f16 {}, {}", top_or_bottom(*top), single(sd), single(sm)),
            Self::Vcvt_SingleToHalf_T1(sd, sm, top) => format!("vcvt{}.f16.f32 {}, {}", top_or_bottom(*top), single(sd), single(sm)),
            Self::Vcvt_FloatToFixed_Single_T1(sd, signed, bits32, frac) => format!("vcvt.{}.f32 {}, {}, #{}", fixed_type(*signed, *bits32), single(sd), single(sd), frac),
            Self::Vcvt_FloatToFixed_Double_T1(dd, signed, bits32, frac) => format!("vcvt.{}.f64 {}, {}, #{}", fixed_type(*signed, *bits32), double(dd), double(dd), frac),
            Self::Vcvt_FixedToFloat_Single_T1(sd, signed, bits32, frac) => format!("vcvt.f32.{} {}, {}, #{}", fixed_type(*signed, *bits32), single(sd), single(sd), frac),
            Self::Vcvt_FixedToFloat_Double_T1(dd, signed, bits32, frac) => format!("vcvt.f64.{} {}, {}, #{}", fixed_type(*signed, *bits32), double(dd), double(dd), frac),
            Self::Blx_Register_T1(rm) => format!("blx {}", gpr(rm)),
            Self::Bx_T1(rm) => format!("bx {}", gpr(rm)),

            // ---- loads / stores (single) ----
            Self::Ldr_Immediate_T1(rt, rn, decoded_imm7) => format!("ldr {}, [{}, {}]", low(rt), low(rn), imm(syntax, *decoded_imm7 as i64)),
            Self::Ldr_Immediate_T2(rt, decoded_imm10) => format!("ldr {}, [sp, {}]", low(rt), imm(syntax, *decoded_imm10 as i64)),
            Self::Ldr_Literal_T1(rt, decoded_imm10) => render_literal_load("ldr", low(rt), instruction_address, *decoded_imm10 as i64, syntax),
            Self::Ldr_Register_T1(rt, rn, rm) => format!("ldr {}, [{}, {}]", low(rt), low(rn), low(rm)),
            Self::Ldrb_Immediate_T1(rt, rn, imm5) => format!("ldrb {}, [{}, {}]", low(rt), low(rn), imm(syntax, *imm5 as i64)),
            Self::Ldrb_Register_T1(rt, rn, rm) => format!("ldrb {}, [{}, {}]", low(rt), low(rn), low(rm)),
            Self::Ldrh_Immediate_T1(rt, rn, decoded_imm6) => format!("ldrh {}, [{}, {}]", low(rt), low(rn), imm(syntax, *decoded_imm6 as i64)),
            Self::Ldrh_Register_T1(rt, rn, rm) => format!("ldrh {}, [{}, {}]", low(rt), low(rn), low(rm)),
            Self::Ldrsb_Register_T1(rt, rn, rm) => format!("ldrsb {}, [{}, {}]", low(rt), low(rn), low(rm)),
            Self::Ldrsh_Register_T1(rt, rn, rm) => format!("ldrsh {}, [{}, {}]", low(rt), low(rn), low(rm)),
            Self::Str_Immediate_T1(rt, rn, decoded_imm7) => format!("str {}, [{}, {}]", low(rt), low(rn), imm(syntax, *decoded_imm7 as i64)),
            Self::Str_Immediate_T2(rt, decoded_imm10) => format!("str {}, [sp, {}]", low(rt), imm(syntax, *decoded_imm10 as i64)),
            Self::Str_Register_T1(rt, rn, rm) => format!("str {}, [{}, {}]", low(rt), low(rn), low(rm)),
            Self::Strb_Immediate_T1(rt, rn, imm5) => format!("strb {}, [{}, {}]", low(rt), low(rn), imm(syntax, *imm5 as i64)),
            Self::Strb_Register_T1(rt, rn, rm) => format!("strb {}, [{}, {}]", low(rt), low(rn), low(rm)),
            Self::Strh_Immediate_T1(rt, rn, decoded_imm6) => format!("strh {}, [{}, {}]", low(rt), low(rn), imm(syntax, *decoded_imm6 as i64)),
            Self::Strh_Register_T1(rt, rn, rm) => format!("strh {}, [{}, {}]", low(rt), low(rn), low(rm)),

            // ---- loads / stores (multiple) ----
            Self::Ldm_T1(rn, registers) => {
                let writeback = if registers.iter().any(|register| register == rn) { "" } else { "!" };
                format!("ldmia {}{}, {}", low(rn), writeback, low_register_list(registers))
            },
            Self::Stm_T1(rn, registers) => format!("stmia {}!, {}", low(rn), low_register_list(registers)),
            ...
            Self::Mrs_T1(rd, spec_reg) => format!("mrs {}, {}", gpr(rd), special_register(spec_reg)),
            Self::Msr_Register_T1(spec_reg, rn) => format!("msr {}, {}", special_register(spec_reg), gpr(rn)),
            ...
            Self::Mov_Immediate_T3(rd, imm16) => format!("movw {}, {}", gpr(rd), imm(syntax, *imm16 as i64)),
            Self::Movt_T1(rd, imm16) => format!("movt {}, {}", gpr(rd), imm(syntax, *imm16 as i64)),
            Self::Mul_T2(rd, rn, rm) => format!("mul {}, {}, {}", gpr(rd), gpr(rn), gpr(rm)),
            Self::Mla_T1(rd, rn, rm, ra) => format!("mla {}, {}, {}, {}", gpr(rd), gpr(rn), gpr(rm), gpr(ra)),
            Self::Mls_T1(rd, rn, rm, ra) => format!("mls {}, {}, {}, {}", gpr(rd), gpr(rn), gpr(rm), gpr(ra)),
            ...
            Self::Ubfx_T1(rd, rn, lsb, width) => format!("ubfx {}, {}, {}, {}", gpr(rd), gpr(rn), imm(syntax, *lsb as i64), imm(syntax, *width as i64)),
            Self::Sbfx_T1(rd, rn, lsb, width) => format!("sbfx {}, {}, {}, {}", gpr(rd), gpr(rn), imm(syntax, *lsb as i64), imm(syntax, *width as i64)),
            Self::Bfi_T1(rd, rn, lsb, width) => format!("bfi {}, {}, {}, {}", gpr(rd), gpr(rn), imm(syntax, *lsb as i64), imm(syntax, *width as i64)),
            Self::Bfc_T1(rd, lsb, width) => format!("bfc {}, {}, {}", gpr(rd), imm(syntax, *lsb as i64), imm(syntax, *width as i64)),
            Self::Ldr_Immediate_T3(rt, rn, imm12) => format!("ldr.w {}, [{}, {}]", gpr(rt), gpr(rn), imm(syntax, *imm12 as i64)),
            Self::Str_Immediate_T3(rt, rn, imm12) => format!("str.w {}, [{}, {}]", gpr(rt), gpr(rn), imm(syntax, *imm12 as i64)),

            // ---- ARMv7-M batch M7c ----
            Self::Ldrex_T1(rt, rn, offset) => {
                if *offset == 0 { format!("ldrex {}, [{}]", gpr(rt), gpr(rn)) }
                else { format!("ldrex {}, [{}, {}]", gpr(rt), gpr(rn), imm(syntax, *offset as i64)) }
            },
            Self::Strex_T1(rd, rt, rn, offset) => {
                if *offset == 0 { format!("strex {}, {}, [{}]", gpr(rd), gpr(rt), gpr(rn)) }
                else { format!("strex {}, {}, [{}, {}]", gpr(rd), gpr(rt), gpr(rn), imm(syntax, *offset as i64)) }
            },
            Self::Ldrexb_T1(rt, rn) => format!("ldrexb {}, [{}]", gpr(rt), gpr(rn)),
            Self::Strexb_T1(rd, rt, rn) => format!("strexb {}, {}, [{}]", gpr(rd), gpr(rt), gpr(rn)),
            // ARMv8-M load-acquire / store-release: size 0=byte (b), 1=halfword (h), 2=word ("")
            Self::LoadAcquire_T1(size, exclusive, rt, rn) =>
                format!("lda{}{} {}, [{}]", if *exclusive { "ex" } else { "" }, ["b", "h", ""][*size as usize], gpr(rt), gpr(rn)),
            Self::StoreRelease_T1(size, rt, rn) =>
                format!("stl{} {}, [{}]", ["b", "h", ""][*size as usize], gpr(rt), gpr(rn)),
            Self::StoreReleaseExclusive_T1(size, rd, rt, rn) =>
                format!("stlex{} {}, {}, [{}]", ["b", "h", ""][*size as usize], gpr(rd), gpr(rt), gpr(rn)),
            ...
            Self::UnprivLoadStore_T1(load, signed, size, rt, rn, imm8) => {
                let mnemonic = if *load {
                    format!("ldr{}{}t", if *signed { "s" } else { "" }, ["b", "h", ""][*size as usize])
                } else {
            ...
            Self::Strexh_T1(rd, rt, rn) => format!("strexh {}, {}, [{}]", gpr(rd), gpr(rt), gpr(rn)),
            ...
            Self::Mov_Immediate_T2(rd, constant, set_flags) => format!("mov{} {}, {}", wide_flag(*set_flags), gpr(rd), imm(syntax, *constant as i64)),
            Self::Mvn_Immediate_T1(rd, constant, set_flags) => format!("mvn{} {}, {}", wide_flag(*set_flags), gpr(rd), imm(syntax, *constant as i64)),
            Self::And_Immediate_T1(rd, rn, constant, set_flags) => format!("and{} {}, {}, {}", wide_flag(*set_flags), gpr(rd), gpr(rn), imm(syntax, *constant as i64)),
            Self::Bic_Immediate_T1(rd, rn, constant, set_flags) => format!("bic{} {}, {}, {}", wide_flag(*set_flags), gpr(rd), gpr(rn), imm(syntax, *constant as i64)),
            Self::Orr_Immediate_T1(rd, rn, constant, set_flags) => format!("orr{} {}, {}, {}", wide_flag(*set_flags), gpr(rd), gpr(rn), imm(syntax, *constant as i64)),
            Self::Eor_Immediate_T1(rd, rn, constant, set_flags) => format!("eor{} {}, {}, {}", wide_flag(*set_flags), gpr(rd), gpr(rn), imm(syntax, *constant as i64)),
            Self::Add_Immediate_T3(rd, rn, constant, set_flags) => format!("add{} {}, {}, {}", wide_flag(*set_flags), gpr(rd), gpr(rn), imm(syntax, *constant as i64)),
            Self::Sub_Immediate_T3(rd, rn, constant, set_flags) => format!("sub{} {}, {}, {}", wide_flag(*set_flags), gpr(rd), gpr(rn), imm(syntax, *constant as i64)),
            Self::Tst_Immediate_T1(rn, constant) => format!("tst.w {}, {}", gpr(rn), imm(syntax, *constant as i64)),
            Self::Teq_Immediate_T1(rn, constant) => format!("teq.w {}, {}", gpr(rn), imm(syntax, *constant as i64)),
            Self::Cmn_Immediate_T1(rn, constant) => format!("cmn.w {}, {}", gpr(rn), imm(syntax, *constant as i64)),
            Self::Cmp_Immediate_T2(rn, constant) => format!("cmp.w {}, {}", gpr(rn), imm(syntax, *constant as i64)),

            // ---- ARMv7-M batch M7e: emitted WITHOUT `.w` (no narrow form; clang rejects `.w` here) ----
            Self::Adc_Immediate_T1(rd, rn, constant, set_flags) => format!("adc{} {}, {}, {}", flag_suffix(*set_flags), gpr(rd), gpr(rn), imm(syntax, *constant as i64)),
            Self::Sbc_Immediate_T1(rd, rn, constant, set_flags) => format!("sbc{} {}, {}, {}", flag_suffix(*set_flags), gpr(rd), gpr(rn), imm(syntax, *constant as i64)),
            Self::Rsb_Immediate_T2(rd, rn, constant, set_flags) => format!("rsb{} {}, {}, {}", flag_suffix(*set_flags), gpr(rd), gpr(rn), imm(syntax, *constant as i64)),
            Self::Orn_Immediate_T1(rd, rn, constant, set_flags) => format!("orn{} {}, {}, {}", flag_suffix(*set_flags), gpr(rd), gpr(rn), imm(syntax, *constant as i64)),

            // ---- ARMv7-M batch M7f: data processing (shifted register), always `.w` ----
            Self::Add_Register_T3(rd, rn, rm, shift, set_flags) => format!("add{} {}, {}, {}{}", wide_flag(*set_flags), gpr(rd), gpr(rn), gpr(rm), render_shift(shift)),
            Self::Sub_Register_T2(rd, rn, rm, shift, set_flags) => format!("sub{} {}, {}, {}{}", wide_flag(*set_flags), gpr(rd), gpr(rn), gpr(rm), render_shift(shift)),
            Self::And_Register_T2(rd, rn, rm, shift, set_flags) => format!("and{} {}, {}, {}{}", wide_flag(*set_flags), gpr(rd), gpr(rn), gpr(rm), render_shift(shift)),
            Self::Orr_Register_T2(rd, rn, rm, shift, set_flags) => format!("orr{} {}, {}, {}{}", wide_flag(*set_flags), gpr(rd), gpr(rn), gpr(rm), render_shift(shift)),
            Self::Eor_Register_T2(rd, rn, rm, shift, set_flags) => format!("eor{} {}, {}, {}{}", wide_flag(*set_flags), gpr(rd), gpr(rn), gpr(rm), render_shift(shift)),
            Self::Bic_Register_T2(rd, rn, rm, shift, set_flags) => format!("bic{} {}, {}, {}{}", wide_flag(*set_flags), gpr(rd), gpr(rn), gpr(rm), render_shift(shift)),
            ...
            Self::Mov_Register_T3(rd, rm, shift, set_flags) => render_mov_register(rd, rm, shift, *set_flags),
            Self::Mvn_Register_T2(rd, rm, shift, set_flags) => format!("mvn{} {}, {}{}", wide_flag(*set_flags), gpr(rd), gpr(rm), render_shift(shift)),
            // ADC/SBC/RSB/ORN register accept (and render with) `.w`
            Self::Adc_Register_T2(rd, rn, rm, shift, set_flags) => format!("adc{} {}, {}, {}{}", wide_flag(*set_flags), gpr(rd), gpr(rn), gpr(rm), render_shift(shift)),
            Self::Sbc_Register_T2(rd, rn, rm, shift, set_flags) => format!("sbc{} {}, {}, {}{}", wide_flag(*set_flags), gpr(rd), gpr(rn), gpr(rm), render_shift(shift)),
            Self::Rsb_Register_T1(rd, rn, rm, shift, set_flags) => format!("rsb{} {}, {}, {}{}", wide_flag(*set_flags), gpr(rd), gpr(rn), gpr(rm), render_shift(shift)),
            Self::Orn_Register_T1(rd, rn, rm, shift, set_flags) => format!("orn{} {}, {}, {}{}", wide_flag(*set_flags), gpr(rd), gpr(rn), gpr(rm), render_shift(shift)),
            Self::Tst_Register_T2(rn, rm, shift) => format!("tst.w {}, {}{}", gpr(rn), gpr(rm), render_shift(shift)),
            Self::Teq_Register_T1(rn, rm, shift) => format!("teq.w {}, {}{}", gpr(rn), gpr(rm), render_shift(shift)),
            Self::Cmn_Register_T2(rn, rm, shift) => format!("cmn.w {}, {}{}", gpr(rn), gpr(rm), render_shift(shift)),
            Self::Cmp_Register_T3(rn, rm, shift) => format!("cmp.w {}, {}{}", gpr(rn), gpr(rm), render_shift(shift)),

            // ---- ARMv7-M batch M7h: wide byte/half load/store + register offset ----
            Self::Ldrb_Immediate_T2(rt, rn, imm12) => format!("ldrb.w {}, [{}, {}]", gpr(rt), gpr(rn), imm(syntax, *imm12 as i64)),
            Self::Strb_Immediate_T2(rt, rn, imm12) => format!("strb.w {}, [{}, {}]", gpr(rt), gpr(rn), imm(syntax, *imm12 as i64)),
            Self::Ldrh_Immediate_T2(rt, rn, imm12) => format!("ldrh.w {}, [{}, {}]", gpr(rt), gpr(rn), imm(syntax, *imm12 as i64)),
            Self::Strh_Immediate_T2(rt, rn, imm12) => format!("strh.w {}, [{}, {}]", gpr(rt), gpr(rn), imm(syntax, *imm12 as i64)),
            Self::Ldrsb_Immediate_T1(rt, rn, imm12) => format!("ldrsb.w {}, [{}, {}]", gpr(rt), gpr(rn), imm(syntax, *imm12 as i64)),
            Self::Ldrsh_Immediate_T1(rt, rn, imm12) => format!("ldrsh.w {}, [{}, {}]", gpr(rt), gpr(rn), imm(syntax, *imm12 as i64)),
            Self::Ldr_Register_T2(rt, rn, rm, lsl) => format!("ldr.w {}, {}", gpr(rt), render_register_offset(rn, rm, *lsl)),
            Self::Str_Register_T2(rt, rn, rm, lsl) => format!("str.w {}, {}", gpr(rt), render_register_offset(rn, rm, *lsl)),
            Self::Ldrb_Register_T2(rt, rn, rm, lsl) => format!("ldrb.w {}, {}", gpr(rt), render_register_offset(rn, rm, *lsl)),
            Self::Strb_Register_T2(rt, rn, rm, lsl) => format!("strb.w {}, {}", gpr(rt), render_register_offset(rn, rm, *lsl)),
            Self::Ldrh_Register_T2(rt, rn, rm, lsl) => format!("ldrh.w {}, {}", gpr(rt), render_register_offset(rn, rm, *lsl)),
            Self::Strh_Register_T2(rt, rn, rm, lsl) => format!("strh.w {}, {}", gpr(rt), render_register_offset(rn, rm, *lsl)),
            Self::Ldrsb_Register_T2(rt, rn, rm, lsl) => format!("ldrsb.w {}, {}", gpr(rt), render_register_offset(rn, rm, *lsl)),
            Self::Ldrsh_Register_T2(rt, rn, rm, lsl) => format!("ldrsh.w {}, {}", gpr(rt), render_register_offset(rn, rm, *lsl)),

            // ---- ARMv7-M batch M7k: long multiply ----
            Self::Smull_T1(rdlo, rdhi, rn, rm) => format!("smull {}, {}, {}, {}", gpr(rdlo), gpr(rdhi), gpr(rn), gpr(rm)),
            Self::Umull_T1(rdlo, rdhi, rn, rm) => format!("umull {}, {}, {}, {}", gpr(rdlo), gpr(rdhi), gpr(rn), gpr(rm)),
            Self::Smlal_T1(rdlo, rdhi, rn, rm) => format!("smlal {}, {}, {}, {}", gpr(rdlo), gpr(rdhi), gpr(rn), gpr(rm)),
            Self::Umlal_T1(rdlo, rdhi, rn, rm) => format!("umlal {}, {}, {}, {}", gpr(rdlo), gpr(rdhi), gpr(rn), gpr(rm)),
            Self::Umaal_T1(rdlo, rdhi, rn, rm) => format!("umaal {}, {}, {}, {}", gpr(rdlo), gpr(rdhi), gpr(rn), gpr(rm)),

            // ---- ARMv7-M batch M7l: wide extend (with ROR), wide byte-reverse, saturate ----
            Self::Sxtb_T2(rd, rm, rotation) => format!("sxtb.w {}, {}{}", gpr(rd), gpr(rm), render_rotation(*rotation)),
            Self::Uxtb_T2(rd, rm, rotation) => format!("uxtb.w {}, {}{}", gpr(rd), gpr(rm), render_rotation(*rotation)),
            Self::Sxth_T2(rd, rm, rotation) => format!("sxth.w {}, {}{}", gpr(rd), gpr(rm), render_rotation(*rotation)),
            Self::Uxth_T2(rd, rm, rotation) => format!("uxth.w {}, {}{}", gpr(rd), gpr(rm), render_rotation(*rotation)),
            ...
            Self::Ssat_T1(rd, sat_imm, rn, shift) => format!("ssat {}, {}, {}{}", gpr(rd), imm(syntax, *sat_imm as i64), gpr(rn), render_shift(shift)),
            Self::Usat_T1(rd, sat_imm, rn, shift) => format!("usat {}, {}, {}{}", gpr(rd), imm(syntax, *sat_imm as i64), gpr(rn), render_shift(shift)),

            // ---- ARMv7-M batch M7i: indexed load/store, LDRD/STRD, literal loads, preload ----
            Self::Ldr_Immediate_T4(rt, rn, off, mode) => format!("ldr {}, {}", gpr(rt), render_indexed(rn, *off as i64, *mode, syntax)),
            Self::Str_Immediate_T4(rt, rn, off, mode) => format!("str {}, {}", gpr(rt), render_indexed(rn, *off as i64, *mode, syntax)),
            Self::Ldrb_Immediate_T3(rt, rn, off, mode) => format!("ldrb {}, {}", gpr(rt), render_indexed(rn, *off as i64, *mode, syntax)),
            Self::Strb_Immediate_T3(rt, rn, off, mode) => format!("strb {}, {}", gpr(rt), render_indexed(rn, *off as i64, *mode, syntax)),
            Self::Ldrh_Immediate_T3(rt, rn, off, mode) => format!("ldrh {}, {}", gpr(rt), render_indexed(rn, *off as i64, *mode, syntax)),
            Self::Strh_Immediate_T3(rt, rn, off, mode) => format!("strh {}, {}", gpr(rt), render_indexed(rn, *off as i64, *mode, syntax)),
            Self::Ldrsb_Immediate_T2(rt, rn, off, mode) => format!("ldrsb {}, {}", gpr(rt), render_indexed(rn, *off as i64, *mode, syntax)),
            Self::Ldrsh_Immediate_T2(rt, rn, off, mode) => format!("ldrsh {}, {}", gpr(rt), render_indexed(rn, *off as i64, *mode, syntax)),
            Self::Ldrd_Immediate_T1(rt, rt2, rn, off, mode) => format!("ldrd {}, {}, {}", gpr(rt), gpr(rt2), render_indexed(rn, *off as i64, *mode, syntax)),
            Self::Strd_Immediate_T1(rt, rt2, rn, off, mode) => format!("strd {}, {}, {}", gpr(rt), gpr(rt2), render_indexed(rn, *off as i64, *mode, syntax)),
            // literal loads carry `.w` so the assembler does not pick the narrow PC-relative form
            Self::Ldr_Literal_T2(rt, off) => format!("ldr.w {}, [pc, {}]", gpr(rt), imm(syntax, *off as i64)),
            Self::Ldrb_Literal_T1(rt, off) => format!("ldrb.w {}, [pc, {}]", gpr(rt), imm(syntax, *off as i64)),
            Self::Ldrh_Literal_T1(rt, off) => format!("ldrh.w {}, [pc, {}]", gpr(rt), imm(syntax, *off as i64)),
            Self::Ldrsb_Literal_T1(rt, off) => format!("ldrsb.w {}, [pc, {}]", gpr(rt), imm(syntax, *off as i64)),
            Self::Ldrsh_Literal_T1(rt, off) => format!("ldrsh.w {}, [pc, {}]", gpr(rt), imm(syntax, *off as i64)),
            Self::Pld_Immediate_T1(rn, off) => format!("pld [{}, {}]", gpr(rn), imm(syntax, *off as i64)),
            Self::Pli_Immediate_T1(rn, off) => format!("pli [{}, {}]", gpr(rn), imm(syntax, *off as i64)),
            ...
            Self::Ldmia_T2(rn, writeback, registers) => {
                if is_sp_register(rn) && *writeback {
                    ...
                    format!("ldm.w {}{}, {}", gpr(rn), writeback_suffix(*writeback), register_list(registers))
                }
            },
            Self::Stmia_T2(rn, writeback, registers) => format!("stm.w {}{}, {}", gpr(rn), writeback_suffix(*writeback), register_list(registers)),
            Self::Ldmdb_T1(rn, writeback, registers) => format!("ldmdb {}{}, {}", gpr(rn), writeback_suffix(*writeback), register_list(registers)),
            Self::Stmdb_T1(rn, writeback, registers) => {
                if is_sp_register(rn) && *writeback {
                    format!("push.w {}", register_list(registers))
                } else {
                    format!("stmdb {}{}, {}", gpr(rn), writeback_suffix(*writeback), register_list(registers))
                }
            },
            ...
            Self::Tt_T1(rd, rn, a, t) => format!("tt{}{} {}, {}", if *a { "a" } else { "" }, if *t { "t" } else { "" }, gpr(rd), gpr(rn)),
            ...
            Self::MveIntArith(op, size, qd, qn, qm) =>
                format!("{}.{}{} {}, {}, {}", op.mnemonic(), op.type_prefix(), size.width_digits(), mve_q(qd), mve_q(qn), mve_q(qm)),
            Self::MveBitwise(op, qd, qn, qm) =>
                format!("{} {}, {}, {}", op.mnemonic(), mve_q(qd), mve_q(qn), mve_q(qm)),
            Self::MveFloatArith(op, size, qd, qn, qm) =>
                format!("{}.f{} {}, {}, {}", op.mnemonic(), size.width_digits(), mve_q(qd), mve_q(qn), mve_q(qm)),

            // ---- MVE vector-by-scalar (Qd, Qn, Rm) and VDUP (Qd, Rt) ----
            Self::MveVecScalarInt(op, size, qd, qn, rm) =>
                format!("{}.{}{} {}, {}, {}", op.mnemonic(), op.type_prefix(), size.width_digits(), mve_q(qd), mve_q(qn), gpr(rm)),
            Self::MveVecScalarFloat(op, size, qd, qn, rm) =>
                format!("{}.f{} {}, {}, {}", op.mnemonic(), size.width_digits(), mve_q(qd), mve_q(qn), gpr(rm)),
            ...
            Self::MveShiftImm(op, size, amount, qd, qm) => {
                ...
                format!("{}.{} {}, {}, #{}", op.mnemonic(), type_part, mve_q(qd), mve_q(qm), amount)
            },
            Self::MveModifiedImmediate(cmode, op, imm8, qd) => mve_modified_immediate(*cmode, *op, *imm8, &mve_q(qd)),
            ...
            Self::MveMisc2(op, size, qd, qm) => {
                ...
                format!("{}.{} {}, {}", op.mnemonic(), type_part, mve_q(qd), mve_q(qm))
            },
            Self::MveMisc2Float(op, size, qd, qm) =>
                format!("{}.f{} {}, {}", op.mnemonic(), size.width_digits(), mve_q(qd), mve_q(qm)),
            Self::MveVmaxaMina(is_min, size, qda, qm) =>
                format!("{}.s{} {}, {}", if *is_min { "vmina" } else { "vmaxa" }, size.width_digits(), mve_q(qda), mve_q(qm)),
            Self::MveVmaxnmaMinnma(is_min, size, qda, qm) =>
                format!("{}.f{} {}, {}", if *is_min { "vminnma" } else { "vmaxnma" }, size.width_digits(), mve_q(qda), mve_q(qm)),
            ...
            Self::MveLoadStore(is_load, size, qd, rn, offset, mode) => {
                ...
                let mnemonic = if *is_load { format!("vldr{}.u{}", letter, width) } else { format!("vstr{}.{}", letter, width) };
                format!("{} {}, {}", mnemonic, mve_q(qd), render_indexed(rn, *offset as i64, *mode, syntax))
            },
            Self::MveGatherScatter(is_load, unsigned, esize, msize, scaled, qd, rn, qm) => {
                let letter = match *msize { 8 => "b", 16 => "h", 32 => "w", _ => "d" };
                let mnemonic = if *is_load {
                    format!("vldr{}.{}{}", letter, if *unsigned { 'u' } else { 's' }, esize)
                } else {
                    format!("vstr{}.{}", letter, esize) // the suffix is the element size (e.g. vstrb.16 = byte access of 16-bit lanes)
                };
                let addr = if *scaled {
                    format!("[{}, {}, uxtw #{}]", gpr(rn), mve_q(qm), crate::enums::mve_mem_size_log(*msize))
                } else {
            ...
            Self::MveGatherScatterBase(is_load, is_dword, writeback, qd, qn, offset) => {
                ...
                let addr = if *offset == 0 { format!("[{}]{}", mve_q(qn), wb) } else { format!("[{}, #{}]{}", mve_q(qn), offset, wb) };
                format!("{} {}, {}", mnemonic, mve_q(qd), addr)
            },
            Self::MveInterleave(is_load, is_quad, pass, size, qd, rn, writeback) => {
                let count: u32 = if *is_quad { 4 } else { 2 };
                let list: Vec<String> = (0..count)
                    .map(|i| mve_q(&crate::enums::Arm32MveVectorRegister::from_field((qd.field() + i) & 7)))
                    .collect();
                let mnemonic = format!("{}{}{}.{}", if *is_load { "vld" } else { "vst" }, count, pass, size.width_digits());
                let wb = if *writeback { "!" } else { "" };
            ...
            Self::LobStart(is_while, tp_size, rn, offset) => {
                let mnemonic = match tp_size {
                    None => if *is_while { "wls".to_string() } else { "dls".to_string() },
                    Some(size) => format!("{}.{}", if *is_while { "wlstp" } else { "dlstp" }, size),
                };
                if *is_while {
                    match instruction_address {
                        Some(address) => format!("{} lr, {}, 0x{:08x}", mnemonic, gpr(rn), pc_relative_target(address, *offset as i64, false)),
                        None => format!("{} lr, {}, {}", mnemonic, gpr(rn), imm(syntax, *offset as i64)),
                    }
                } else {
            ...
            Self::LobEnd(tail_predicated, offset) => {
                ...
                match instruction_address {
                    Some(address) => format!("{} lr, 0x{:08x}", mnemonic, pc_relative_target(address, *offset as i64, false)),
            ...
            Self::MveReduce(op, size, rd, qm) =>
                format!("{}.{}{} {}, {}", op.mnemonic(), op.type_prefix(), size.width_digits(), gpr(rd), mve_q(qm)),
            Self::MveVabav(signed, size, rd, qn, qm) =>
                format!("vabav.{}{} {}, {}, {}", if *signed { 's' } else { 'u' }, size.width_digits(), gpr(rd), mve_q(qn), mve_q(qm)),
            Self::MveDualMac(subtract, exchange, accumulate, unsigned, size, rda, qn, qm) => {
                let mnemonic = format!("v{}{}{}",
                    if *subtract { "mlsdav" } else { "mladav" },
                    if *accumulate { "a" } else { "" },
                    if *exchange { "x" } else { "" });
                format!("{}.{}{} {}, {}, {}", mnemonic, if *unsigned { 'u' } else { 's' }, size.width_digits(), gpr(rda), mve_q(qn), mve_q(qm))
            },
            Self::MveLongDualMac(op, exchange, accumulate, unsigned, size, rda_lo, rda_hi, qn, qm) => {
                let mnemonic = format!("{}{}{}", op.mnemonic(), if *accumulate { "a" } else { "" }, if *exchange { "x" } else { "" });
                format!("{}.{}{} {}, {}, {}, {}", mnemonic, if *unsigned { 'u' } else { 's' }, size.width_digits(), gpr(rda_lo), gpr(rda_hi), mve_q(qn), mve_q(qm))
            },
            Self::MveVmovTwoLane(to_vector, idx1, qd, rt, rt2) => {
                ...
                let (lane_hi, lane_lo) = (format!("{}[{}]", mve_q(qd), idx1), format!("{}[{}]", mve_q(qd), idx1.saturating_sub(2)));
                if *to_vector {
            ...
            Self::MveVcvtFloatInt(to_int, unsigned, size, qd, qm) => {
                ...
                if *to_int {
                    format!("vcvt.{}{}.f{} {}, {}", int_type, width, width, mve_q(qd), mve_q(qm))
                } else {
                    format!("vcvt.f{}.{}{} {}, {}", width, int_type, width, mve_q(qd), mve_q(qm))
                }
            },
            Self::MveVcvtFixed(to_fixed, unsigned, size, fbits, qd, qm) => {
                ...
                let int_type = if *unsigned { 'u' } else { 's' };
                if *to_fixed {
                    format!("vcvt.{}{}.f{} {}, {}, #{}", int_type, width, width, mve_q(qd), mve_q(qm), fbits)
                } else {
                    format!("vcvt.f{}.{}{} {}, {}, #{}", width, int_type, width, mve_q(qd), mve_q(qm), fbits)
                }
            },
            Self::MveVcvtHalf(top, half_to_single, qd, qm) => {
                let types = if *half_to_single { "f32.f16" } else { "f16.f32" };
                format!("vcvt{}.{} {}, {}", if *top { "t" } else { "b" }, types, mve_q(qd), mve_q(qm))
            },
            Self::MveShiftNarrow(op, unsigned, top, src_is_32, shift, qd, qm) => {
                use crate::enums::Arm32MveShiftNarrowOp::*;
                let type_prefix = match op { Vshrn | Vrshrn => 'i', Vqshrn | Vqrshrn => if *unsigned { 'u' } else { 's' }, Vqshrun | Vqrshrun => 's' };
                let src = if *src_is_32 { 32 } else { 16 };
                format!("{}{}.{}{} {}, {}, #{}", op.mnemonic(), if *top { "t" } else { "b" }, type_prefix, src, mve_q(qd), mve_q(qm), shift)
            },
            // width-changing register moves
            Self::MveVmovl(top, unsigned, size, qd, qm) =>
                format!("vmovl{}.{}{} {}, {}", if *top { "t" } else { "b" }, if *unsigned { 'u' } else { 's' }, size.width_digits(), mve_q(qd), mve_q(qm)),
            Self::MveVmovn(top, size, qd, qm) =>
                format!("vmovn{}.i{} {}, {}", if *top { "t" } else { "b" }, size.width_digits(), mve_q(qd), mve_q(qm)),
            Self::MveVqmovn(kind, unsigned, top, size, qd, qm) => {
                ...
                format!("{}{}.{}{} {}, {}", mnem, if *top { "t" } else { "b" }, type_letter, size.width_digits(), mve_q(qd), mve_q(qm))
            },
            Self::MveVmull(polynomial, unsigned, top, size, qd, qn, qm) => {
                let type_letter = if *polynomial { 'p' } else if *unsigned { 'u' } else { 's' };
                format!("vmull{}.{}{} {}, {}, {}", if *top { "t" } else { "b" }, type_letter, size.width_digits(), mve_q(qd), mve_q(qn), mve_q(qm))
            },
            Self::MveVmulh(rounding, unsigned, size, qd, qn, qm) =>
                format!("v{}mulh.{}{} {}, {}, {}", if *rounding { "r" } else { "" }, if *unsigned { 'u' } else { 's' }, size.width_digits(), mve_q(qd), mve_q(qn), mve_q(qm)),
            Self::MveVqdmull(top, size32, qd, qn, qm) =>
                format!("vqdmull{}.s{} {}, {}, {}", if *top { "t" } else { "b" }, if *size32 { 32 } else { 16 }, mve_q(qd), mve_q(qn), mve_q(qm)),
            Self::MveVqdmullScalar(top, size32, qd, qn, rm) =>
                format!("vqdmull{}.s{} {}, {}, {}", if *top { "t" } else { "b" }, if *size32 { 32 } else { 16 }, mve_q(qd), mve_q(qn), gpr(rm)),
            Self::MveVqdmladh(subtract, rounding, exchange, size, qd, qn, qm) =>
                format!("v{}{}{}.s{} {}, {}, {}",
                    if *rounding { "qrd" } else { "qd" },
                    if *subtract { "mlsdh" } else { "mladh" },
                    if *exchange { "x" } else { "" },
                    size.width_digits(), mve_q(qd), mve_q(qn), mve_q(qm)),
            ...
            Self::MveShiftByVector(rounding, saturating, unsigned, size, qd, qm, qn) =>
                format!("v{}{}shl.{}{} {}, {}, {}",
                    if *saturating { "q" } else { "" }, if *rounding { "r" } else { "" },
                    if *unsigned { 'u' } else { 's' }, size.width_digits(), mve_q(qd), mve_q(qm), mve_q(qn)),
            Self::MveShiftByScalar(rounding, saturating, unsigned, size, qda, rm) =>
                format!("v{}{}shl.{}{} {}, {}",
                    if *saturating { "q" } else { "" }, if *rounding { "r" } else { "" },
                    if *unsigned { 'u' } else { 's' }, size.width_digits(), mve_q(qda), gpr(rm)),
            Self::MveVshll(top, unsigned, size, shift, qd, qm) =>
                format!("vshll{}.{}{} {}, {}, #{}", if *top { "t" } else { "b" },
                    if *unsigned { 'u' } else { 's' }, size.width_digits(), mve_q(qd), mve_q(qm), shift),
            Self::MveVaddlv(accumulate, unsigned, rd_lo, rd_hi, qm) =>
                format!("vaddlv{}.{}32 {}, {}, {}", if *accumulate { "a" } else { "" }, if *unsigned { 'u' } else { 's' }, gpr(rd_lo), gpr(rd_hi), mve_q(qm)),
            // complex-number ops: `<op>.<type> Qd, Qn, Qm, #<rotation>`
            Self::MveVcaddInt(halving, size, rot270, qd, qn, qm) => {
                let (mnemonic, type_part) = if *halving { ("vhcadd", format!("s{}", size.width_digits())) } else { ("vcadd", format!("i{}", size.width_digits())) };
                format!("{}.{} {}, {}, {}, #{}", mnemonic, type_part, mve_q(qd), mve_q(qn), mve_q(qm), if *rot270 { 270 } else { 90 })
            },
            Self::MveVcaddFloat(size, rot270, qd, qn, qm) =>
                format!("vcadd.f{} {}, {}, {}, #{}", size.width_digits(), mve_q(qd), mve_q(qn), mve_q(qm), if *rot270 { 270 } else { 90 }),
            Self::MveVcmul(size, rotate, qd, qn, qm) =>
                format!("vcmul.f{} {}, {}, {}, #{}", size.width_digits(), mve_q(qd), mve_q(qn), mve_q(qm), (*rotate as u32) * 90),
            Self::MveVcmla(size, rotate, qd, qn, qm) =>
                format!("vcmla.f{} {}, {}, {}, #{}", size.width_digits(), mve_q(qd), mve_q(qn), mve_q(qm), (*rotate as u32) * 90),
            Self::MveVpsel(qd, qn, qm) => format!("vpsel {}, {}, {}", mve_q(qd), mve_q(qn), mve_q(qm)),
            Self::MveVadc(subtract, init_carry, qd, qn, qm) =>
                format!("v{}c{}.i32 {}, {}, {}", if *subtract { "sb" } else { "ad" }, if *init_carry { "i" } else { "" }, mve_q(qd), mve_q(qn), mve_q(qm)),
            ...
            Self::MveViddup(decrement, size, qd, rn, wrap_rm, step) => {
                let mnemonic = match (*decrement, wrap_rm.is_some()) {
                    (false, false) => "vidup", (true, false) => "vddup",
                    (false, true) => "viwdup", (true, true) => "vdwdup",
                };
                match wrap_rm {
                    Some(rm) => format!("{}.u{} {}, {}, {}, #{}", mnemonic, size.width_digits(), mve_q(qd), gpr(rn), gpr(rm), step),
                    None => format!("{}.u{} {}, {}, #{}", mnemonic, size.width_digits(), mve_q(qd), gpr(rn), step),
                }
            },
            Self::MveVbrsr(size, qd, qn, rm) =>
                format!("vbrsr.{} {}, {}, {}", size.width_digits(), mve_q(qd), mve_q(qn), gpr(rm)),
            ...
            Self::MveVcmpReg(cond, size, qn, qm) =>
                format!("vcmp.{}{} {}, {}, {}", cond.type_prefix(), size.width_digits(), cond.mnemonic(), mve_q(qn), mve_q(qm)),
            Self::MveVcmpScalar(cond, size, qn, rm) =>
                format!("vcmp.{}{} {}, {}, {}", cond.type_prefix(), size.width_digits(), cond.mnemonic(), mve_q(qn), gpr(rm)),
            Self::MveVcmpFloatReg(cond, size, qn, qm) =>
                format!("vcmp.f{} {}, {}, {}", size.width_digits(), cond.mnemonic(), mve_q(qn), mve_q(qm)),
            Self::MveVcmpFloatScalar(cond, size, qn, rm) =>
                format!("vcmp.f{} {}, {}, {}", size.width_digits(), cond.mnemonic(), mve_q(qn), gpr(rm)),
            ...
            Self::MveVptReg(cond, size, qn, qm, mask) =>
                format!("vp{}.{}{} {}, {}, {}", crate::enums::mve_predicate_mask_suffix(*mask), cond.type_prefix(), size.width_digits(), cond.mnemonic(), mve_q(qn), mve_q(qm)),
            Self::MveVptScalar(cond, size, qn, rm, mask) =>
                format!("vp{}.{}{} {}, {}, {}", crate::enums::mve_predicate_mask_suffix(*mask), cond.type_prefix(), size.width_digits(), cond.mnemonic(), mve_q(qn), gpr(rm)),
            Self::MveVptFloatReg(cond, size, qn, qm, mask) =>
                format!("vp{}.f{} {}, {}, {}", crate::enums::mve_predicate_mask_suffix(*mask), size.width_digits(), cond.mnemonic(), mve_q(qn), mve_q(qm)),
            Self::MveVptFloatScalar(cond, size, qn, rm, mask) =>
                format!("vp{}.f{} {}, {}, {}", crate::enums::mve_predicate_mask_suffix(*mask), size.width_digits(), cond.mnemonic(), mve_q(qn), gpr(rm)),
            Self::MveFloatReduce(op, size, rd, qm) =>
                format!("{}.f{} {}, {}", op.mnemonic(), size.width_digits(), gpr(rd), mve_q(qm)),
            Self::MveVcvtRound(rounding, unsigned, size, qd, qm) => {
                let mode = ['a', 'n', 'p', 'm'][(*rounding & 0b11) as usize];
                let width = size.width_digits();
                format!("vcvt{}.{}{}.f{} {}, {}", mode, if *unsigned { 'u' } else { 's' }, width, width, mve_q(qd), mve_q(qm))
            },
        }
    }
}
